  ...
  if (user.phone === 'none') {
    ...
    if (user.ban) {
      ...
      if (tx === "🔙Ortga" || tx === "🔙Назад") {
        ...
        sm("📊Statistika", Markup.inlineKeyboard([productBtns]))
      } else if (tx === '📚Bot haqida' || tx === "📚О боте") {

      }
      if (tx === "👥Foydalanuvchilar" || tx === "👥Пользователи") {
...
bot.on("callback_query", async (msg) => {
  ...
  const sendM = (userID, text) => {
    msg.telegram.sendMessage(userID, text, {parse_mode: "HTML"})
  }
  msg.deleteMessage();
  if (data === "back") {
    setStep(id, { step: "none" })
    let btns = [];
    let all = await userModel.find();
    for (let k of all) {
      btns.push([{ text: k.name, callback_data: "users__" + k.id }]);
    ...
    userModel.findOne({ id: userID }).then((data) => {
      sm(
        ...
        Markup.inlineKeyboard([
          ...
          [
            {
              text: !data.ban ? "⭕️Bloklash" : "⭕️Blokdan olish",
    ...
    sm(messages.onblock, keyboard.back);
    sendM(
      userID,
      messages.freedom
    );
    ...
    sm(messages.offblock, keyboard.back);
    sendM(
      userID,
      messages.blocked
    );
    ...
    userModel.findOne({ id: userID }).then((data) => {
      sm(
        `💰Hisob raqamidagi pullar: ${data.balance} UZS`,
        Markup.inlineKeyboard([
          [{ text: "💳Hisobni to'ldirish", callback_data: "pay__" + data.id }, { text: "💸Pul yechib olish", callback_data: "get__" + data.id }],
          [{ text: "🔙Ortga", callback_data: "back" }]
        ]),
        keyboard.back
      );
